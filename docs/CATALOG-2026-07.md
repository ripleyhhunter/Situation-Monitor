...
| 14 | **Camera imagery overhaul** (added 2026-07-04, camera sweep part 2) | both | M | WeatherBug DMV network (30 cams, page-embedded roster, stable `wwc.instacam.com/instacamimg/{ID}/{ID}_l.jpg` stills — verified minutes-fresh); USGS HIVIS river-gauge cams (NIMS API keyless — the SPA's embedded x-api-key returns 403 while keyless returns 200; 17 DC + 2 Boise incl. Boise R. at Glenwood = the BIGI1 flood gauge); landmark roster refresh (5 dead pruned incl. the removed WeatherBug trio + old White House YT; earthTV WH `r1K7DyQn3jg`, Union Station railcam `0Q1pN-KpNZc`, EarthCam Monument YT `oDCAAfOSqvA`, FOX5 PW Marina, NPS ARD Mall air cam with direct still, 5 National Zoo Wowza HLS cams added); **dcCamerasFetcher REMOVED** — the DDOT dataset is a 2021 pole inventory with zero image URLs (314 pins that looked like cameras but linked nowhere; DC-proper street imagery no longer exists publicly: DDOT viewer dead, TrafficLand keyed). EarthCam Capitol page cam skipped (stacks on the Senate official cam at identical coords). |
| 13 | **DC camera coverage: VDOT 511 + Arlington ITS + PG TRIP** (added 2026-07-04, camera sweep) | dc | M | All adversarially verified keyless. VDOT 511 internal roster (`/services/map/array/cameras`, ~1,674 statewide GeoJSON features) → 464 active DC-metro cams with ~45s snapshot stills (`snapshot.vdotcameras.com`, derive by stripping `/thumbs/` + `.flv` from image_url) AND Wowza HLS (CORS *). Arlington County ArcGIS Open_Data roster → 284 ONLINE cams, HLS-only, per-camera URL field authoritative (ports vary; one sign-flipped-longitude row dropped). PG County TRIP (carsprogram.org 511x) → county camNNN feeds only (CHARTFeed relays deduped against mdchart; broken-flagged skipped). In-modal playback via hls.js (dynamic import; native HLS on Safari). |
