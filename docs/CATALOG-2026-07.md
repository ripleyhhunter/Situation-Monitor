...
| 4 | **Boise crime upgrade: Ada County CrimeMapper** | boise | M | Replaces `bpd-crime`. Verified live: point geometry, records through 2026-07-02 on a 2026-07-04 fetch (~1-2 day lag vs BPD_Crimes_Public's ~5-week lag), and covers four agencies (Boise PD, Ada County Sheriff, Meridian PD, Garden City PD) — the whole county instead of city-only. One row per offense → dedupe by Case_Number. |
